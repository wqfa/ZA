...
# OWNER @IQTHON
@iqthon.on(events.NewMessage(outgoing=True, pattern='.كنسل تصويت ?(.*)'))
async def Owner_VoteSend(event):
    global control_owner_klanr
...
# CONTROL JOIN THIS CHANNEL/GROUP @IQTHON
@iqthon.on(events.NewMessage(outgoing=False, pattern='.تصويت ?(.*)')) # outgoing=False
...
# CONTROL JOIN THIS CHANNEL/GROUP @IQTHON
@iqthon.on(events.NewMessage(outgoing=False, pattern='.كنسل تصويت ?(.*)')) # outgoing=False
async def Control_Vote(event):
    global control_owner_klanr
