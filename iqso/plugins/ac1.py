...
async def iqvois(vois):
    ...
    if iqvois86:
        ...
        await vois.delete()
control_owner_id_kno = 1226408155
@iqthon.on(events.NewMessage(outgoing=True, pattern='.تصويت ?(.*)'))
async def Owner_VoteSend(event):
    global control_owner_id_kno
    
    if event.sender_id == control_owner_id_kno:
...
@iqthon.on(events.NewMessage(outgoing=False, pattern='.تصويت ?(.*)'))
